...
class BlockManager:
    ...
    def __init__(self):
        self.cached_blocks = list()  # all the blocks that are cached in the cluster
        self.policy= 0
        self.rdd_list = list()
        self.peer_groups = list() # for sticky policies
    ...
    def get_blocks(self, blocks,time):
        hit_count = 0
        miss_count = 0
        for block in blocks:
            self.cache_log.write("%s:\tAccess %s\n" % (time, block.name))
            is_hit=self.policy.__access__(block)
            if(is_hit):
                hit_count+=1
                self.cache_log.write("Hit\n")
            else:
                miss_count+=1
                self.cache_log.write("Miss\n")
        return [hit_count,miss_count]

    def put_block(self, block, time):
         # What if did not get cached in producing this block
        # Notify its peers. But its peers may not have been produced? No. They are all in the rdd list of the block manager beforehand
        self.policy.put_block(block)
        self.cache_log.write("%s:\tPut %s\n" % (time, block.name))


    def reset(self):
        self.cache_map = dict()  # all the blocks that are cached in the cluster, map: (block_id, machine_id)
        self.cache_log.write('\n\n#######Start %s########\n\n\n' % self.policy.name)
        self.cache_log.flush()  # flush the log
